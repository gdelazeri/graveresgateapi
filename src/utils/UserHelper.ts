...
export const generateRegistrationId = (registrationId?: string) => {
  ...
  const registrationNumber = parseInt(registrationId.replace(REGISTRATION_PREFIX, '')) + 1;
  const formattedRegistrationNumber = registrationNumber.toString().padStart(3, '0');
