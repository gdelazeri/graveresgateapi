...
const dutyRequestPositionRepository = DataSource.getRepository(DutyRequestPosition);

export async function createDutyRequestPosition(input: any) {
  try {
    return dutyRequestPositionRepository.save(dutyRequestPositionRepository.create(input));
  } catch (error) {
    throw error;
  }
}
